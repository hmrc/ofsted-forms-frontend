...
//                    log.info(
//                      s"""|$id - something in database, step in URI, user revisiting old page, render filled in form
//                          |\t\t data: $o
...
//               (implicit ec: ExecutionContext, witnessEither: Either[Result, ?] <= PlayStack, witnessState: State[(DB, List[String]), ?] <= PlayStack): Future[Result] = {
//
